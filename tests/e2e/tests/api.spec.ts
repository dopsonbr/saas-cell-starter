...
async function getFreshSessionToken(page: Page) {
  ...
  if (!token) throw new Error('Unable to obtain Clerk session token for API E2E')
  return token
}
...
test('authenticated dashboard request reaches the dedicated database', async ({ page, request }) => {
...
test('authenticated invalid writes are rejected by contract validation', async ({ page, request }) => {
  const token = await getFreshSessionToken(page)
