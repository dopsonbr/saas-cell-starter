...
export function createCustomerAuthMiddleware(options: CustomerAuthOptions) {
  ...
  return createMiddleware<{ Variables: CustomerAuthVariables }>(async (c, next) => {
    ...
    if (!auth.userId || !auth.orgId) return c.json({ error: 'Organization required' }, 403)
    if (auth.orgId !== options.expectedOrgId) return c.json({ error: 'Wrong customer organization' }, 403)
